...
Seven steps, about two frames each (`IDLE_BLINK_STEP_MS`, 24fps twos): `01-open` → `02-closing` → `03-half` → `04-462-blink` → `03-half` → `02-closing` → `01-open`. The body bitmap stays `idle.png` the whole way.
