...
describe("voice card prompt", () => {
  it("is installed as RAI_SYSTEM", () => {
    assert.match(RAI_SYSTEM, /You are Star Rai\. Not an assistant/);
    assert.match(RAI_SYSTEM, /Never kiss/);
    assert.doesNotMatch(RAI_SYSTEM, /\bkiss is a blown kiss/i);
    assert.match(RAI_SYSTEM, /middle_finger/);
    assert.match(RAI_SYSTEM, /pose omitted = keep current sheet/);
  });
});
