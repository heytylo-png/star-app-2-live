...
| [rai/skeleton.json](./rai/skeleton.json) | Bones, slots, and authored pose clips (idle/talk/wave/scold/pout/shy) |
...
- `?spine=rai`: load `rai/skeleton.json` + `rai/layers/*.png`. Pose clips move bones (wave is not bind pose). If any required layer 404s, **fall back to PNG**.  
