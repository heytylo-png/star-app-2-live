...
A spoken line keeps its talk or mood sheet while she is saying it. Frown `idle.png` is not that line. Once the line is over (speech finished, not still sending), the next rest is the ~3.4s / ~2.8s settle — then the open rest sheet (`idle_blink_01_open.png`, a byte copy of `idle.png`) so the eyes-only blink can run. The transcript row staying on screen does not hold the sheet. Body-sheet crossfade ~380ms. Blink frames hard-cut; they do not crossfade. Talking does not snap a dedicated pose (including `talk`) to frown idle.
