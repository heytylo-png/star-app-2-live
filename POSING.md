...
A spoken line keeps its talk or mood sheet while she is saying it. Frown `idle.png` is not that line. Once the line is over (speech finished, not still sending), the next rest is the ~3.4s / ~2.8s settle — then `idle.png`. Rest blink is parked (`IDLE_BLINK_ENABLED` is false) until a standing full-body clip PASSes. `idle_blink_01_open.png` stays a byte copy of `idle.png` for that cycle (01 → 02 → 03 → 04 → 03 → 02 → 01); it is not the mounted rest sheet while the flag is off. The transcript row staying on screen does not hold the sheet. Body-sheet crossfade ~380ms. Blink frames hard-cut; they do not crossfade. Talking does not snap a dedicated pose (including `talk`) to frown idle.
